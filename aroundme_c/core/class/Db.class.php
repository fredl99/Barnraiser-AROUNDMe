...
class Database {
	...
	function newConnection() {
		...
		if (!is_resource($this->connection)) {
			$GLOBALS['am_error_log'][] = array('db_connect_error', mysql_error());
		}
		else {
	...
	function am_parse($str) {
		...
		// process <code> tags
		// there is a but with nested tags here but I can't figure out a fix - tom
		$pattern = "/<code>(.*?)?<\/code>/s";
		...
		// remove javascript type things
		$str = preg_replace("/<script[^>]*>.*<\/script>|<script[^>]*>/is","",$str);

		/*
		// apply paragraphs
		$paragraph_arr = preg_split("/\s{2,}/s", $str);
		$out = "";
		if (!empty($paragraph_arr)) {
			foreach ($paragraph_arr as $i):
				if (preg_match("/^[^<]/s", $i)) {
					$i = nl2br($i);
					$i = "<p>" . $i . "</p>";
				}
